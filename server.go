...
func (s *server) pushBlockMsg(sp *serverPeer, hash *chainhash.Hash, doneChan chan<- struct{},
	...
	if sendInv {
		dagState := sp.server.chain.DAGSnapshot()
		invMsg := wire.NewMsgInvSizeHint(uint(len(dagState.Tips)))
		for _, tip := range dagState.Tips {
			block, err := sp.server.chain.BlockByHash(&tip)
			if err != nil {
				peerLog.Errorf("Failed to retrieve block for %v: %v", tip, err)
			}

			iv := wire.NewInvVect(wire.InvTypeBlock, &tip, block.Height())
			invMsg.AddInvVect(iv)
		}
